...
def main():
    print("[Nagster] Starting Nagster Agent...")
    ...
    if cfg.get("remember_me") and cfg.get("employee_id"):
        emp = cfg["employee_id"]
        print(f"[Nagster] Using saved Employee ID: {emp}")
    ...
    if not employee_id:
        print("[Nagster] Prompting for Employee ID...")
        employee_id, _ = prompt_employee_id_popup(cfg)
        backend_base = cfg.get("backend_base_url", backend_base)

    print(f"[Nagster] Initializing agent for employee: {employee_id}")
